# OPENSTUDIOHUB
# Module: src/interfaces/qt/components/pipeline_wizard.py
# Role: Sequential visual component for the Production Manager
...
class PipelineStepNode(QWidget):
    """Individual node of the progress bar (circle + title)."""

    clicked = Signal(int)

    def __init__(self, step_number: int, title: str) -> None:
        super().__init__()
    ...
    def set_state(self, is_active: bool, is_completed: bool) -> None:
        if is_completed:
...
class PipelineWizardWidget(QFrame):
    """Sequential progress bar and batch creation orchestrator."""
